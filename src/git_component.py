...
def args_parse():
    ...
    sp = subparsers.add_parser(
        "current_version",
        help="Compute current git version ",
    )
    sp.set_defaults(cmd="current_version")

    sp = subparsers.add_parser(
        "get_name",
        help="Get the component name",
    )
    sp.set_defaults(cmd="get_name")
...
class GitComponent:
    ...
    def __init__(self, args):
        ...
        self.abs_location_root = None
        self.locations = []
        self.full_locations = []
        self.just_copy_files = []
        self.git_files = []
    ...
    def __check_changed_files_against_paths_to_check(
        ...
        for check_path in paths_to_check:
            if os.path.isdir(os.path.join(self.abs_location_root, check_path)):
                check_path = (
                    os.path.relpath(
                        os.path.join(self.abs_location_root, check_path),
                        self.abs_location_root,
                    )
                    + "/"
                )
    ...
    def _cmd_get_name(self):
        print(self.name)

    def _cmd_current_version(self):
        print(self.__get_git_version())
                ...
                if isinstance(scripts, list):
                    ...
                    for dependency in depends:
                        if dependency in already_executed_actions:
                            raise Exception(
                                f"Circular dependency detected: Action list '{action}' depends on '{dependency}' but '{dependency}' already executed ..."
                            )
    ...
    def _cmd_run_on_new_version(self):
        ...
        def _execute_one_action(action):
            ...
            scripts_res, scripts_duration = self._run_scripts(scripts)
            print(
                f"Action list '{action}' of component={self.name!r} has "
    ...
    def _cmd_gen_changelog(self):
        ...
        if not inst_scripts or len(inst_scripts) == 0:
            ...
            print(
                f"{tests_type.capitalize()} of component={self.name!r} has "
                f"{'succeeded' if scripts_res == 0 else 'FAILED'}"
            )
            if scripts_res != 0:
                return scripts_res
    ...
    def _cmd_pack(self):
        ...
        for action, fpath in self.package_actions.items():
            shutil.copy(
                os.path.join(self.abs_location_root, fpath),
